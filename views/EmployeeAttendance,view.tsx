...
import { EmployeeAttendanceWrapper } from "@/features/dashboard/attendance/EmployeeAttendanceWrapper"
import {
  EmployeeAttendance,
...
export const EmployeeAttendanceView = async () => {
  const cookiesStore = await cookies()
  const [attendanceRes, summaryRes] = await Promise.all([
    fetch(
      `${BASEURL}/dashboard/attendance?month=${new Date().getMonth() + 1}&year=${new Date().getFullYear()}`,
      {
        headers: {
    ...
    fetch(`${BASEURL}/dashboard/attendance/stats`, {
      headers: {
        "Content-Type": "application/json",
        Cookie: cookiesStore.toString(),
      },
